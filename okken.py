...
try:
    # ### 変更点: Secretsのキー名を新しい定義に合わせる ###
    service_account_string = st.secrets["FIREBASE_ADMIN_SDK_CREDS"]
    if not service_account_string: # Noneまたは空文字でないかチェック
        raise KeyError("FIREBASE_ADMIN_SDK_CREDS")
    ...
    try:
        ...
        if not isinstance(firebase_service_account_info, dict):
            st.error("Secret 'FIREBASE_ADMIN_SDK_CREDS' の内容がJSONオブジェクト形式ではありません。")
            st.stop()
    except json.JSONDecodeError as json_err:
        # JSON文字列のパースに失敗した場合のエラー
        st.error(f"Secret 'FIREBASE_ADMIN_SDK_CREDS' のJSON形式が不正です: {json_err}")
        st.error("サービスアカウントキーのJSON文字列全体が正しくコピー＆ペーストされているか確認してください（特に引用符や改行）。")
        st.stop()

except KeyError:
    # ### 変更点: エラーメッセージのキー名を合わせる ###
    st.error("Secret 'FIREBASE_ADMIN_SDK_CREDS' が見つかりません。Secrets設定を確認してください。")
    st.stop()

try:
    # ### 変更点: Secretsのキー名を新しい定義に合わせる ###
    web_config_string = st.secrets["FIREBASE_WEB_CONFIG"]
    if not web_config_string: # Noneまたは空文字でないかチェック
        raise KeyError("FIREBASE_WEB_CONFIG")
    ...
    try:
        ...
        if not isinstance(firebase_config, dict):
             st.error("Secret 'FIREBASE_WEB_CONFIG' の内容がJSONオブジェクト形式ではありません。")
             st.stop()
        # # Optional: Add messagingSenderId if missing and needed
        # if "messagingSenderId" not in firebase_config:
        #      # ... (messagingSenderIdを補完するロジック、必要なら) ...
        #      print("Warning: messagingSenderId missing from FIREBASE_WEB_CONFIG secret.")
    except json.JSONDecodeError as json_err:
        # JSON文字列のパースに失敗した場合のエラー
        st.error(f"Secret 'FIREBASE_WEB_CONFIG' のJSON形式が不正です: {json_err}")
        st.error("Firebase Web設定のJSON文字列全体が正しくコピー＆ペーストされているか確認してください。")
        st.stop()

except KeyError:
     # ### 変更点: エラーメッセージのキー名を合わせる ###
    st.error("Secret 'FIREBASE_WEB_CONFIG' が見つかりません。Secrets設定を確認してください。")
    st.stop()
...
# --- Question Definitions (Needed for default loading logic) ---
question_definitions = [
    # ... (Include the full question_definitions list from your original code here) ...
    # Example structure:
...
def load_and_set_default_preferences(user_id: str, question_definitions: List[Dict]):
    # ... (function content unchanged, assuming question_definitions is passed correctly) ...
...
# ============================================================
#  5. Authentication Handling & Session State (Keep as is)
# ============================================================
if 'user_info' not in st.session_state: st.session_state['user_info'] = None
if 'id_token' not in st.session_state: st.session_state['id_token'] = None
# Initialize other necessary states early if needed
keys_to_init_early = [
    ("nickname", ""),
    ("main_menu", "新しい旅を計画する"),
    ("defaults_loaded", False),
]
for key, default in keys_to_init_early:
    if key not in st.session_state: st.session_state[key] = default

# --- Authentication Check and Login Form (Logic unchanged) ---
if st.session_state['user_info'] is None:
    st.subheader("Googleアカウントでログイン")
    st.write("Okosy を利用するには、Googleアカウントでのログインが必要です。")
    st.info("アカウントをお持ちでない場合は、Signinボタンをクリックしてください。") # Minor text change
    if auth_obj is None:
        ...
        if login_result and isinstance(login_result, dict) and login_result.get('success') is True:
            ...
            if id_token:
                ...
                try:
                    ...
                    st.error(f"ログインエラー (トークン検証失敗): {e}"); print(f"Token verify failed: {e}")
                    st.session_state['id_token'] = None; st.session_state['user_info'] = None # Clear invalid state
            else:
                st.error("ログイン成功しましたが、トークンが見つかりません。"); print("AccessToken not found.")
...
# ============================================================
#  6. Post-Login Setup (Sidebar etc. - Keep as is)
# ============================================================
if st.session_state.get('user_info') is not None:
    user_id = st.session_state['user_info'].get('uid')
    if not user_id:
        st.error("ユーザーID取得不可"); st.session_state['user_info'] = None; st.session_state['id_token'] = None; st.rerun()

    # --- Sidebar Settings (After Login) ---
    st.sidebar.header("メニュー")
    user_email = st.session_state['user_info'].get('email', '不明')
    nickname = st.session_state.get('nickname', '') # Use initialized value
    if nickname:
        st.sidebar.write(f"ログイン中: {nickname} さん")
    ...
    if st.sidebar.button("ログアウト"):
        ...
        keys_to_clear = [
            # ... (Your extensive list of keys to clear) ...
             ...
             "confirmed_destination" # Make sure all relevant keys are listed
        ]
    ...
    st.sidebar.markdown("---")
    # Use the session state key for the radio button
    menu_choice = st.sidebar.radio(
        "操作を選択", # Added label
        ["新しい旅を計画する", "過去の旅のしおりを見る"],
        key="main_menu",
        label_visibility="collapsed"
        )
    st.sidebar.image("assets/logo_okosy.png", width=100) # Ensure path is correct in repo

    # --- Placeholder for Main Content ---
    print(f"--- User {user_id} logged in. Selected menu: {menu_choice} ---")
    # The rest of your application logic (Google API helpers, OpenAI calls, UI based on menu_choice)
    # would follow here, unchanged in principle unless they also had file path dependencies.
